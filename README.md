...
이 글은 제가 스테레오 이벤트 카메라 모듈과 FPGA 보드를 고정하는 브라켓을 설계하고 3D 프린터로 출력하면서 남긴 기록입니다. 처음 만들 때 확인한 치수, 나사와 케이블 공간, 슬라이서 설정, 출력 뒤 조립 과정에서 알게 된 내용을 순서대로 정리했습니다.
...
출력물은 생각보다 너무 두껍게 만들 필요가 없다. 0.4 mm 노즐 기준으로 벽 두께를 최소 1.2 mm 이상, 하중이 걸리는 리브·보스 주변은 1.6~2.4 mm 정도로 시작하면 PLA에서도 충분히 단단한 경우가 많다. 큰 판을 통째로 두껍게 하는 대신 다음처럼 보강하는 편이 가볍고 출력도 안정적이다.
...
카메라 고정 나사의 중심 간격은 이번 모듈에서 26.61 mm로 사용했다. 이 값은 해당 카메라 모듈용 규격이므로, 다른 보드는 반드시 실제 나사 중심 간격으로 교체한다. FPGA 보드의 네 고정 위치, USB 포트, MIPI/FFC 커넥터, 후면 포트 쪽 케이블 배출 공간은 조립 더미 모델에서 매번 확인한다.

## 6. 나사와 레일은 작은 시험편부터 출력하기

전체 브라켓을 출력하기 전에 아래 요소만 포함한 작은 시험편을 만든다.

- 목표 구멍 지름을 0.1~0.2 mm씩 바꾼 나사 구멍 3~5개
- 실제 나사와 너트가 들어가는 보스 한 개
- PCB 두께에 맞춘 짧은 레일 조각
- 필요하면 케이블이 통과하는 홈 한 개

시험편으로 확인할 항목은 다음과 같다.

1. 나사가 손으로 들어가는지, 너무 헐겁지 않은지
2. 너트가 뒤쪽에서 들어가고 도구로 조일 수 있는지
3. PCB가 레일에 얹혔을 때 흔들리지 않는지
4. 레일이 보드 모서리를 과도하게 누르지 않는지
5. FFC 또는 USB 플러그가 실제 방향으로 빠져나오는지

이 단계를 건너뛰면 큰 브라켓에서 구멍 하나가 조금 틀어진 이유로 전체를 다시 출력하게 된다. 체결 시험편은 짧게는 수십 분 안에 결과를 볼 수 있어 가장 효율적인 확인 방법이다.
...
아래 출력 세트는 FPGA 보드 베이스 1개와 카메라 홀더 2개를 한 세트로 구성한 예시다. 조립할 때는 FPGA 보드을 베이스 후면의 네 고정점에 나사로 체결하고, 카메라 홀더 두 개는 베이스의 반복 결합 구멍 중 원하는 위치에 각각 고정한다. 홀더 위치를 바꾸면 두 카메라 렌즈 사이의 baseline을 바꿀 수 있으며, 보드가 바뀌면 카메라 홀더만 새로 출력할 수 있다.
...
<p align="center">
  <img src="assets/assembled-rig-rear.jpg" width="155" alt="카메라 홀더와 고정 구조">
</p>
...
<p align="center">
  <img src="assets/assembled-rig-rear.jpg" width="155" alt="FPGA 보드 베이스와 카메라 홀더 결합 구조">
</p>

FPGA 보드 베이스는 보드의 네 나사 위치를 기준으로 FPGA 보드을 지지하고, 앞쪽의 반복 결합 구멍 grid로 두 카메라 홀더 위치를 조절한다. 카메라 홀더는 앞뒤 두 줄의 구멍을 사용해 네 점으로 고정하므로, 단순히 한 줄 구멍에 끼운 구조보다 회전과 흔들림에 강하다. 중앙의 바닥 고정 구멍은 삼각대 또는 다른 장착판에 연결할 때 사용한다.
...
<p align="center">
  <img src="assets/slicer-layout.png" width="155" alt="브라켓 출력 세트 배치">
</p>
