...
이 글은 스테레오 이벤트 카메라를 위해 PCB 모듈을 원하는 형태로 지지하고, 두 카메라 사이의 baseline을 조절할 수 있는 구조를 직접 설계·출력하면서 남긴 기록입니다. 이벤트 카메라 모듈과 FPGA 보드를 실제로 조립하는 데 필요한 치수, 나사와 케이블 공간, 슬라이서 설정, 출력 뒤 확인한 내용을 아래 순서대로 정리했습니다.

<p align="center">
  <img src="assets/assembled-stereo-rig.jpg" width="155" alt="스테레오 이벤트 카메라 모듈과 FPGA 보드 조립 상태">
</p>

<p align="center"><sub>스테레오 이벤트 카메라 모듈과 FPGA 보드 조립 상태</sub></p>
...
## 3. 더미 모델로 실제 조립을 미리 확인하기

실제 부품을 단순 더미로 만들어 브라켓 설계에 함께 넣으면, 전체를 출력하기 전에 나사·렌즈·케이블 간섭을 확인할 수 있다. 실물 외관을 전부 재현할 필요는 없고, 조립에 영향을 주는 위치와 공간만 맞추면 된다.

### 이번 설계의 더미 모델 예시

<p align="center">
  <img src="assets/camera-module-dummy.png" width="270" alt="카메라 모듈 더미 모델">
  <img src="assets/fpga-board-dummy.png" width="270" alt="FPGA 보드 더미 모델">
</p>

카메라 더미에는 PCB 외곽과 두께, 렌즈 중심, 나사 구멍, FFC 케이블 공간을 넣었다. FPGA 보드 더미에는 네 고정 구멍과 USB·FFC·케이블 공간을 넣었다. 이렇게 두면 브라켓이 나사 구멍을 막는지, 렌즈 중심과 baseline이 맞는지, 포트와 케이블이 지나갈 공간이 남는지를 화면에서 먼저 확인할 수 있다.

특히 케이블은 선 한 가닥이 아니라 폭과 굽힘 공간이 있는 부피로 잡아야 한다. FFC는 폭과 굽힘 반지름을, USB와 전원 케이블은 플러그와 케이블이 빠져나갈 공간을 함께 넣는다. 이 여유 공간을 keepout 형상으로 두면 출력 후 케이블 때문에 다시 만드는 일을 줄일 수 있다.
...
<p align="center">
  <img src="assets/boss-taper-example.png" width="220" alt="고깔형 나사 보스 예시">
</p>

고깔형 보스 예시: 앞쪽은 PCB 나사 구멍에만 닿고, 뒤쪽은 넓어져 지지 구조와 연결된다.
...
나사로 조인 면이 바닥이나 다른 장착판에 평평하게 닿아야 한다면, 나사 머리나 너트가 들어갈 자리파기(counterbore 또는 recess)도 함께 만든다. 그렇지 않으면 돌출된 나사 머리나 너트 때문에 조립면이 뜨고 기울어진다.

<p align="center">
  <img src="assets/counterbore-example.png" width="220" alt="나사 머리 자리파기 예시">
</p>

자리파기 예시: 나사 머리나 너트가 안쪽으로 들어가 조립면이 평평하게 닿는다.

### 리브와 두께

이번 출력에서는 카메라 홀더 보강 벽의 두께를 3 mm, 가로로 긴 벽의 두께를 약 5 mm로 두었고 충분히 단단했다. 다만 이 값은 이번 재료와 하중에서 확인한 경험값이므로, 다른 형상은 실제 조립·하중 조건으로 시험 출력해 확인한다.

<p align="center">
  <img src="assets/holder-rib-rail-example.png" width="250" alt="카메라 모듈 홀더의 리브와 하단 레일 예시">
</p>

카메라 모듈 홀더 측면 예시: 하단 레일이 PCB를 받으며, 옆의 사선 리브가 긴 벽과 보스를 지지한다.
...
<p align="center">
  <img src="assets/camera-holder-module-example.png" width="260" alt="카메라 모듈 고정 홀더 예시">
  <img src="assets/fpga-base-module-example.png" width="260" alt="FPGA 보드 고정 베이스 예시">
</p>

왼쪽은 카메라 모듈을 레일과 나사 보스로 고정하는 홀더이고, 오른쪽은 FPGA 보드를 고정하면서 카메라 홀더 위치를 조절하는 베이스다.

이렇게 나누면 카메라 홀더만 다시 출력해도 되고, 다른 카메라를 쓰게 되었을 때 베이스는 유지한 채 홀더만 새로 설계할 수 있다. 출력 실패도 작은 부품에서 먼저 잡을 수 있어 재료 낭비가 줄어든다.

## 5. 나중에 다시 사용할 나사 기준

아래는 이번 조립에서 실제로 사용한 나사와 구멍 기준이다. 회사 나사 보관함에서 비슷한 나사를 새로 고르기보다, 현재 조립품에서 나사 하나를 빼서 지름·길이·머리 형상을 맞춰 같은 것을 다시 사용한다. 다른 PCB에 적용할 때만 실제 부품을 기준으로 다시 확인한다.
