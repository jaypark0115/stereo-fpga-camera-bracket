...
나사로 조인 면이 바닥이나 다른 장착판에 평평하게 닿아야 한다면, 나사 머리나 너트가 들어갈 **자리파기(counterbore 또는 recess)**도 함께 만든다. 그렇지 않으면 돌출된 나사 머리나 너트 때문에 조립면이 뜨고 기울어진다. 이번 STL에는 홀더와 베이스를 잇는 5 mm 결합부의 하단 자리파기(깊이 5 mm), 삼각대 결합부 상단 자리파기(깊이 3 mm), FPGA 보드 고정 나사 쪽의 공구·나사 머리 공간이 포함되어 있다. 사용할 나사의 머리와 너트 형상에 맞춰 깊이와 지름을 다시 확인한다.

### 리브와 두께

이번 STL에서 실제로 사용한 두께는 베이스 바닥 **14 mm**, 카메라 홀더 바닥 **8 mm**, 카메라 PCB를 받는 하단 레일 깊이 **5 mm**다. 카메라 홀더의 얇은 보강 벽은 **3.0 mm**로 두었고, 이번 PLA 출력물에서는 이 정도만으로도 충분히 단단했다. 가로로 길게 이어지는 벽은 약 **5 mm**로 두어도 문제없이 사용했다. 다만 이 수치는 이번 출력물의 재료와 하중에서 확인한 경험값이므로, 다른 형상에서는 실제 조립·하중 조건을 기준으로 시험 출력해 확인하는 과정이 중요하다. PCB 나사 구멍에 닿는 고깔형 접촉 ring은 **1.2 mm**부터 시작해 뒤쪽 지지부에서 **3.0 mm**로 넓어지게 만들었다. 즉, 큰 판을 통째로 두껍게 만든 것이 아니라 실제 체결·지지 위치에만 이 두께를 사용한 구조다.
