...
이번 STL에서 실제로 사용한 두께는 베이스 바닥 **14 mm**, 카메라 홀더 바닥 **8 mm**, 카메라 PCB를 받는 하단 레일 깊이 **5 mm**다. 카메라 홀더의 얇은 보강 벽은 **3.0 mm**로 두었고, 이번 PLA 출력물에서는 이 정도만으로도 충분히 단단했다. PCB 나사 구멍에 닿는 고깔형 접촉 ring은 **1.2 mm**부터 시작해 뒤쪽 지지부에서 **3.0 mm**로 넓어지게 만들었다. 즉, 큰 판을 통째로 두껍게 만든 것이 아니라 실제 체결·지지 위치에만 이 두께를 사용한 구조다.
